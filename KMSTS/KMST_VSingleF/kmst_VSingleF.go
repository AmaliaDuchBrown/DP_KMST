...
//Input stage
func input(istream string, outr chan<- request, outl chan<- []edge) {
...
// Generator stage
func generator(inr <-chan request, inl <-chan []edge, outr1 chan<- request, outl1 chan<- []edge) {
	for {
		r, ok := <-inr
		if ok {
			switch r.op {
			case insert, update:
				...
				go filter(inr, outr, inl, outl, r.e)
				inr = outr
				inl = outl
			case delete:
			case graph, kmst:
...
func filter(inr <-chan request, outr chan<- request,
	inl <-chan []edge, outl chan<- []edge, e edge) {
	var root int16
	root = e.x //e.x is the filter id-parameter
	//filter memory
	//var adjv []int16 //adjacents nodes
...
func main() {
	...
	start := time.Now()
	go input(istream, inr, inl) // Launch Input.
	go generator(inr, inl, outr, outl)
